...
import me.arasple.mc.trchat.neoforge.protocol.RedisEnvelopeCodec;
...
public final class RedisBridge implements AutoCloseable {
    ...
    public void start() {
        ...
        subscriberThread = Thread.ofPlatform()
            .daemon(true)
            .name("TrChat NeoForge Redis subscriber")
    ...
    public boolean publish(TrChatMessage message) {
        ...
        synchronized (this) {
            try {
                ...
                closePublisher();
                TrChatNeoForge.LOGGER.warn("Redis publish failed: {}", exception.getMessage());
                return false;
            }
        }
    }
    ...
    private void subscriptionLoop() {
        while (running.get()) {
            try {
                ...
                subscribed = true;
                TrChatNeoForge.LOGGER.info("Connected to Redis at {}:{} on channel '{}'",
                ...
                if (running.get()) {
                    TrChatNeoForge.LOGGER.warn("Redis subscription lost: {}", exception.getMessage());
                }
            } finally {
                subscribed = false;
                closeSubscriber();
    ...
    private void receive(String payload) {
        try {
            ...
            TrChatNeoForge.LOGGER.warn("Ignoring malformed TrChat Redis message: {}", exception.getMessage());
        }
    }
